...
export default function EntriesTable({
  ...
  onTogglePage: (pageIds: number[], selected: boolean) => void;
  ...
  const checkboxClass =
    "h-4 w-4 cursor-pointer rounded border-line accent-stone-700";
            ...
            return (
              <tr
                ...
                className={`align-top transition ${
                  selected ? "bg-sand/70" : "hover:bg-sand/50"
                }`}
              >
                <td className="px-4 py-3">
                  <input
                    type="checkbox"
                    aria-label={`Select the ${entry.date} entry for ${entry.client_name}`}
                    className={checkboxClass}
                    checked={selected}
